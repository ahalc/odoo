...
function makeActionManager(env) {
    ...
    async function _triggerDownload(action, options, type) {
        ...
        if (type === "xlsx") {
            return _downloadReportXLSX(url, action).then(
                function () {
     ...
     * @param {ActionOptions} options
     */
     function _downloadReportXLSX(url, actions) {
        var self = this;
        framework.blockUI();
        var type = "xlsx";
...
//                        throw new Error(`crash_manager rpc_error ${error}`);
                    reject();
                },
    ...
    async function _executeReportAction(action, options) {
        ...
        if (action.report_type === "qweb-html") {
            ...
            return _triggerDownload(action, options, "xlsx");
        }
        else {
            console.error(
